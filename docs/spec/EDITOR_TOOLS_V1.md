...
| 6 | `refresh_assets` | edit | 15000/300000 |
...
1. `AssetDatabase.Refresh(ImportAssetOptions.ForceSynchronousImport)` を Unity メインスレッドで実行する。
2. ディスク上の変更を Editor に反映する。
3. Plugin は Refresh 後、同一フレームで `EditorApplication.isCompiling` を確認し、`compiling` フラグ付きで即座にレスポンスを返す（コンパイル完了を待たない）。
4. `compiling=true` の場合、Server 側で `get_editor_state` をポーリングし、コンパイル完了まで待機してから MCP レスポンスを返す。`compiling=false` の場合はポーリングをスキップする。
5. コンパイルエラーがある場合は `errors` フィールドにエラー内容を含める。
...
```json
{
  ...
  "test_name_pattern": "",
  "errors": [
    { "type": "error", "message": "Assets/Scripts/Foo.cs(10,5): error CS1002: ; expected" }
  ]
}
```
...
6. `errors`: コンパイルエラー等がある場合のエラーエントリ配列（`refresh_assets` と同形式）。コンパイル成功時は省略
...
8. テスト実行前に `refresh_assets` を実行し、Server 側でコンパイル完了を待機する。コンパイルエラーがある場合はテスト実行をスキップし、`errors` フィールドにエラー内容を返す。
...
1. `type: "image"` — Plugin 側でダウンスケール済みの base64 PNG（長辺 960px 以下、`mimeType: "image/png"`）
...
`width`/`height` はディスクに保存されたフルサイズ PNG の実寸。インライン画像はダウンスケールされている場合がある。

### 10.3 動作ルール

1. `source` 省略時は `game_view`。
2. `source=game_view`:
   - **Play Mode 必須**。Game View をフォーカスし `RepaintImmediately` で同期リペイントした後、`ScreenCapture.CaptureScreenshotAsTexture()` で合成出力（全カメラ・Canvas UI・ポストプロセス含む）をキャプチャ。解像度は Game View ウィンドウの実サイズに従う。`camera_path` は無視される。
   - **Edit Mode**: `ERR_INVALID_STATE`。エラーメッセージで `source='scene_view'` または `control_play_mode` の使用を案内する。
3. `source=scene_view`:
   - `SceneView.lastActiveSceneView` からカメラを取得し、Scene View ウィンドウサイズで `RenderTexture` レンダリング。Scene View が開いていない場合は `ERR_INVALID_STATE`。
4. `source=camera`:
   - `camera_path` 必須。指定されたカメラ 1 台をカメラのピクセルサイズでレンダリング。`camera_path` 省略時は `ERR_INVALID_PARAMS`。
5. `output_path` 省略時は `<project>/Screenshots/unity_screenshot_<timestamp>.png`。親ディレクトリが存在しない場合は自動作成する。
6. フルサイズ PNG をディスクに保存した後、インライン用に長辺 960px 以下にダウンスケールして base64 エンコードする。
...
| `ERR_INVALID_STATE` | Play Mode 外で `pause` 実行、Play Mode 外で `game_view` キャプチャ、Scene View 未開放で `scene_view` キャプチャ | `control_play_mode`, `capture_screenshot` |
| `ERR_OBJECT_NOT_FOUND` | `camera_path` で指定したオブジェクトが見つからない、または Camera コンポーネントがない | `capture_screenshot` |
