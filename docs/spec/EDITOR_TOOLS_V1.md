...
|---|---|---|---|---|---|
| 1 | `get_editor_state` | read-only | sync | 5000/10000 | true |
| 2 | `get_play_mode_state` | read-only | sync | 5000/10000 | true |
| 3 | `control_play_mode` | edit | sync | 10000/30000 | false |
| 4 | `read_console` | read-only | sync | 10000/30000 | true |
| 5 | `clear_console` | edit | sync | 5000/10000 | false |
| 6 | `refresh_assets` | edit | sync | 30000/120000 | false |
| 7 | `run_tests` | edit | job | 300000/1800000 | false |
| 8 | `manage_job` | read-only | sync | 10000/15000 | false |
| 9 | `capture_screenshot` | read-only | sync | 15000/60000 | true |
...
テスト実行後、最大 `job_long_poll_ms`（5秒）まで Plugin 側で完了を待機する。
待機中にテストが完了した場合は `state` が終端状態（`succeeded`/`failed`）で `result` を含む。
完了しなかった場合は非終端状態（`queued`/`running`）で `result` は `null`。
...
1. `mode` 省略時は `all`。
2. `supports_cancel=true`。実行中のテストをキャンセルできる。
3. 受理時に `job_id` を発行する。
4. Plugin 側で最大 `job_long_poll_ms`（5秒）待機し、テストが完了していれば結果を直接返す。
5. 5秒以内に完了しない場合は非終端状態で返し、以降は `manage_job(wait)` で結果をポーリングする。
6. テストが早期に完了した場合は5秒待たずに即座に応答する。
...
`wait` は `get_status` と同一形式で返す。違いは、非終端状態の場合に Plugin 側で最大 `job_long_poll_ms`（5秒）待機してから応答する点。待機中にジョブが完了した場合は即座に応答する。
...
2. `get_status` は副作用なし・再試行可。即座に応答する。
3. `wait` は副作用なし。非終端状態の場合、Plugin 側で最大 `job_long_poll_ms`（5秒）待機する。待機中にジョブが終端状態に遷移した場合は即座に応答する。MCP クライアントは終端状態になるまで `wait` を繰り返し呼び出す。
4. `cancel` は `supports_cancel=true` の job に対してキャンセル要求を送信する。
5. 終端状態の job に `cancel` した場合は `rejected` を返す。
6. v1 では進捗 push メッセージを定義しないため、`progress` は `null` でもよい。
7. 不正な `action` は `ERR_INVALID_PARAMS`。
...
## 11. `capture_screenshot` (read-only)
...
| `ERR_JOB_NOT_FOUND` | 未知の `job_id` | `manage_job` |
| `ERR_CANCEL_NOT_SUPPORTED` | キャンセル非対応の job への `cancel` | `manage_job` |
...
| ファイル | 内容 |
|---|---|
| `Server/ToolCatalog.cs` L20-162 | 全 9 ツールのスキーマ・メタデータ定義 |
...
| ファイル | 内容 |
|---|---|
| `UnityMCPPlugin/.../CommandExecutor.cs` L23-91 | `read_console`, `get_editor_state`, `get_play_mode_state`, `clear_console`, `refresh_assets`, `control_play_mode` の実行ロジック |
