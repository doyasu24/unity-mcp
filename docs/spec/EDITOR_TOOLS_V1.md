...
2. `source=game_view`:
   - **Play Mode 必須**。`ScreenCapture.CaptureScreenshotAsTexture()` で Game View の合成出力（全カメラ・Canvas UI・ポストプロセス含む）をキャプチャ。`width`/`height`/`camera_path` は無視される（Game View の実解像度で取得）。
