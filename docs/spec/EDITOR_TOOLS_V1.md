...
| 9 | `execute_batch` | edit | 120000/600000 |
...
| `atomic` | boolean | リクエストの `atomic` 値 |
| `rolled_back` | boolean | `atomic` かつ失敗時に Undo ロールバックを実行した場合 `true` |

### 動作ルール

- **逐次実行**: 操作は配列順に1つずつ実行される
- **stop_on_error**: エラー発生後の操作は `"skipped"` エラーで記録
- **最大50操作**: `operations` 配列は最大50要素
- **禁止ツール**: 以下のツールはバッチ内で使用不可（`ERR_INVALID_PARAMS`）
  - `execute_batch`（再帰防止）
  - `get_editor_state`
  - `run_tests`
  - `refresh_assets`
- **MCP統一名変換**: `manage_game_object` 等の統一名は `prefab_path` の有無で自動的にワイヤー名に変換

### atomic の制限（ベストエフォート）

`atomic: true` は Unity Undo システムを利用するため、Undo 対応操作のみロールバック可能。

- **Undo 対応**: `manage_game_object`, `manage_component`, `instantiate_prefab`（シーン操作）
- **Undo 非対応（ロールバック不可）**:
  - Prefab 編集（`ApplyModifiedPropertiesWithoutUndo` 使用）
  - AssetDatabase 操作（`manage_asset`）
  - シーン操作（`open_scene`, `create_scene`, `save_scene`）
  - `clear_console`, `control_play_mode`
